  ...
  const addProduct = (e) => {
    ...
    if (productImg) {
      ...
      const storageRef = ref(
        storage,
        productCategory !== 'Сопутствующие товары'
          ? `product-images/fish-color-icon.png`
          : `product-images/_2.jpeg`
      );
      getDownloadURL(storageRef, 'fish-color-icon').then((url) => {
        addDoc(collection(db, 'products'), {
          productName,
          productCategory,
          productUnit,
    ...
    <>
      {
        <div className="flex flex-col justify-between bg-white rounded shadow-lg p-4 sm:p-12 my-auto sm:mx-auto">
          <ToastContainer />
          <h2 className="text-center text-sm sm:text-lg mb-2 font-bold">Добавить товар</h2>
          <form
            autoComplete="off"
          ...
          >
            ...
            />
            <br />
            <label htmlFor="product-price">Цена товара за единицу измерения</label>
            <input
              type="number"
              className="border-2 p-1"
              required
            ...
            <button
              type="submit"
              className="bg-orange-500 text-sm sm:text-base text-white font-bold py-2 px-4 border border-orange-500 rounded cursor-pointer"
            >
              Добавить
            </button>
          </form>
          <button
            onClick={() => navigate('/')}
            className="mt-4 sm:mt-6 text-sm sm:text-base bg-orange-500 text-white font-bold py-2 px-4 border border-orange-500 rounded cursor-pointer"
          >
            Назад в каталог
          </button>
        </div>
